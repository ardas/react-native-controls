...
export default class CodeInput extends PureComponent {
  ...
  getInitialCode = () => {
    const { initialValue, size } = this.props;

    const initialArray = initialValue.substr(0, size).split('');
    const fullSizeInitialArray = initialArray.concat(
      new Array(size - initialArray.length).fill(null)
    );

    return fullSizeInitialArray;
  };
